...
export default function CarouselRing({
  ...
  const handleCardClick = (DiamondRingsID) => {
    navigate(`/ring-detail/${DiamondRingsID}`);
  };
    ...
    >
      ...
      >
        {highestPricedProducts.map((product) => (
          <SwiperSlide key={product.DiamondRingsID}>
            <Card
              onClick={() => handleCardClick(product.DiamondRingsID)}
              key={product.DiamondRingsID}
              hoverable
              cover={
                <img
                  src={product.ImageRings}
                  alt=""
                  style={{
                    width: "100%",
                    height: "250px",
