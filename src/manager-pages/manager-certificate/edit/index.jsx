...
const EditCertificateForm = ({ certificate, onSave, onCancel }) => {
  ...
  const validateCertificatesExistEdit = async (rule, value) => {
    ...
    if (value === products.GIAReportNumber) {
    ...
    if (certificate) {
      form.setFieldsValue({
        InspectionDate: certificate.InspectionDate
          ? moment(certificate.InspectionDate)
          : null,
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8090/certificate/edit-certificate/${certificate.CertificateID}`,
        formattedValues
      );
      ...
      notification.error({
        ...
        description:
          "Failed to update the certificate. Please try again later.",
      });
    }
  };
  ...
  return (
    <Form form={form} onFinish={onFinish} layout="vertical">
      <Form.Item
        ...
        label="GIA Report Number (GIA123)"
        rules={[
      ...
      <Form.Item
        ...
        rules={[
          { required: true, message: "Carat Weight is required" },
          {
            type: "number",
            min: 0,
            message: "Carat Weight must be a positive number",
          },
...
EditCertificateForm.propTypes = {
  certificate: PropTypes.shape({
    ...
    DiamondTimepiecesID: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.number,
    ]),
